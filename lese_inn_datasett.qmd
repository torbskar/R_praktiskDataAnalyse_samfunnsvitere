...
`glimpse` gir også noe ytterligere informasjon, som antall observasjoner i datasettet og hvilken type variablene er. Når det i output står "dbl" betyr at det er en numerisk variabel, og "fct" betyr at det er en faktorvariabel.
...
Her får vi vite at det er `r dim(ungdata_alko)[1]` rader (dvs. observasjoner) og `r dim(ungdata_alko)[2]` kolonner (dvs. variable).
