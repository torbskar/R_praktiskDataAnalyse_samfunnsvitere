...
Her får vi vite at det er `r dim(wagepan)[1]` rader (dvs. observasjoner) og `r dim(wagepan)[2]` kolonner (dvs. variable).
...
Det er lite hensiktsmessig å vise alt i konsollen fordi det rett og slett ikke er plass. Nederst står det derfor angitt at det er flere variable som ikke vises og navnet på de første av disse.
...
```
Vi kan altså også angi både rader og kolonner på denne måten. Her er eksempel som viser første 3 rader og variabelnummer 5 til 8.
...
I denne output'en er den første kolonnen altså variabelnavnene, deretter er det en kolonne som viser hva slags type variabel det er, og deretter de første observasjonene på hver variabel slik at man får et inntrykk av hvordan det ser ut. `glimpse()` gir altså omtrent samme informasjon som `head()`, men er nok mer hensiktsmessig hvis mange variable.
