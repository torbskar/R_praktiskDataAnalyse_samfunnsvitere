...
Legg alltid resultatene i et eget objekt med et navn som er rimelig enkelt å forstå hva er. I følgende kode legges resultatet i et nytt objekt `lm_est1`. Deretter kan man hente ut de delene av resultatet vi er interessert i. I aller første omgang er vi bare interessert i regresjonslinjas konstantledd (startpunktet) og stigningstall. Disse kaller vi vanligvis *regresjonskoeffisienter*. Det kan vi få ut ved å bruke funksjonen `coef`. (Vi kommer tilbake til å se på de fulle resultatene senere, som vi oftest er interessert i).
...
Den generelle regresjonsligningen skrives som $y=a+bx$, der $x$ er forklaringsvariabelen. Regresjonskoeffisienten, $b$, tolkes som forskjellen i gjennomsnittet på utfallsvariabelen, $y$, mellom de som er en enhets forskjell på $x$-variabelen. 
...
## Flere variable
Det er ikke så ofte vi bruker regresjon med bare en forklaringsvariabel, såkalt "enkel lineær regresjon".[^linearregresjon-4] Langt mer vanlig er å bruke flere variable samtidig i det vi kaller "multippel regresjon".[^linearregresjon-5] I multippel regresjon kan man altså beskrive mer kompliserte mønstre i dataene.
...
Merk at konstantleddet er tolkbart som forventet verdi på utfallsvariabelen når alle andre prediktorer er null. I eksempelet ovenfor med timelønn og klassetilhørighet, er det altså ikke noen koeffisient for klasse I. Gjennomsnittlig timelønn i klasse I er når de andre dummyene er 0, altså `r round(coef(est2)[1],1)` kroner. Gjennomsnittlig timelønn for klasse II er tilsvarende `r round(coef(est2)[1],1)` + (`r round(coef(est2)[2],2)`) = `r round(coef(est2)[1],1) + round(coef(est2)[2],1)`.  
...
Kan også lett integreres med andre eksterne verktøy, først og fremst "grammar of tables" i pakken {gt}
Ulempe: 
...
Merk at Word vil vise tabellen med de fonter etc som er forvalgt for Word. Dette kan du endre i Word etterpå. Det er en rekke funksjoner i Word for å formatere tabeller som du kan bruke. 
...
{gtsummary} kan eksportere til følgende formater: Word, rtf, html, latex og markdown. Resultatene kan også lett integreres med andre funksjoner, først og fremst "grammar of tables" i pakken {gt} og {flextable} - altså for mer avanserte ting som vi ikke dekker her.
